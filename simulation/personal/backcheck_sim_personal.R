# Key Information --------------------------------------------------------------
# Title: Backchecking Simulation - Personal Computer Version
...
# # NOTE: NOT RUN and NOT OPTIMIZED; may be very, very slow (could possibly 
#         take *days* to finish)
# ---------------------------------------------------------------------------- #
...
library(doParallel) #for %dopar%

# source function scripts
source("scripts/convenience_functions.R") # for analysis
source("scripts/simulate_Ra_Rb.R") # for creating backchecks
source("scripts/typos.R")

# only works on non-Windows machines
ncores <- detectCores()
# registerDoParallel(as.numeric(ncores %/% 4)) # uncomment to do in parallel

# number of cores to use for fitting Stan model
ncores_stan <- 4

# create directory for output of MC simulations
dir.create("data/MC_Output")
...
## save to directory for access by analysis script
write.csv(mc_params, 'data/mc_params.csv', row.names = F)
...
load("data/surveys/backcheck_survey.RData")
...
for(i in 1:nrow(mc_params)){ # this part could be parallelized as well

  # print script to identify in log
  print(paste('Backcheck Monte Carlo Simulation', i,
              'Started', Sys.time()))
  
  params <- unlist(mc_params[i, ])
  print(params)
  ...
  results <- foreach(j = 1:50, .packages = c('rstan', 'gtools', 'stringdist',
                                             'tidyverse')) %dopar% {
    
    # set Stan options for each worker
    #rstan_options(auto_write = TRUE)
    options(mc.cores = ncores_stan)
    ...
    backcheck_model <- cmdstan_model(stan_file = 'stan_models/MM_SQ_backchecking.stan',
  ...
  # model evaluation
  mod_eval <- parallel::mclapply(results, eval_model_oos)
  ...
  save(mod_eval, qual_eval, weight_eval, JSD, enum_probs, orig_surv,
       file = paste0('data/MC_Output/results',
                              as.numeric(i), '.RData'))
